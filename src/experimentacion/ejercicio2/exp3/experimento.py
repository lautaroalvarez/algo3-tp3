...
archivo_entrada = path + "tp3/src/experimentacion/ejercicio2/exp3/casos.in"
#--constantes
cant_repeticiones = 5
...
sys.stdout.write("\n")
sys.stdout.write("-------------------------------------------------------------\n")
sys.stdout.write("---------------- CORRIDA AUMENTANDO CAPACIDAD ---------------\n")
sys.stdout.flush()

entrada = csv.reader(open(archivo_entrada, "rb"))

archivo_salida = path + "tp3/src/experimentacion/ejercicio2/exp3/corrida.csv"
	...
	if len(fila) > 0:
		...
		for vecindad in vecindades:
			...
			for j in xrange(0, cant_repeticiones):
				...
				sys.stdout.write("ej3 - datos: "+str(header_dat)+" - codigo: "+vecindad+" - rep: "+str(j)+"/"+str(cant_repeticiones)+"\n")
				sys.stdout.flush()
